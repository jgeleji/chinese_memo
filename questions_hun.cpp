...
chinese::questions_hun::questions_hun()
{
}

void chinese::questions_hun::load_file(std::string const& filename)
{
	std::fstream infile;
	...
	while(infile.good())
	{
		...
		hungarian_overlaps[dp.hungarian].insert(loaded_data.size()-1);
		...
		question_index_finder[dp.chinese][dp.hungarian][dp.english] = loaded_data.size() - 1;
...
std::string chinese::questions_hun::to_string(DATATYPE val)
{
	switch(val)
	{
		case DATATYPE_CHINESE:
			return "\033[38;5;226mchinese\033[0m";
		case DATATYPE_HUNGARIAN:
			return "\033[38;5;236mhungarian\033[0m";
...
std::string const& chinese::questions_hun::datapoint::get(DATATYPE which) const
{
...
bool chinese::questions_hun::datapoint::ask(
	chinese::Input input,
	questions_hun::DATATYPE provided,
	questions_hun::DATATYPE asked,
...
void chinese::questions_hun::statistics_screen(
	std::map<questions_hun::q_type, std::pair<size_t, double>> const& recurrence_scores,
	int breaks
) const
{
	...
	for(auto iter = recurrence_scores.begin(); iter != recurrence_scores.end(); ++iter)
	{
		...
		if(false && score < -4)
		{
			...
			statusfile.open("status_hun.txt", std::fstream::out | std::fstream::app);
			statusfile << loaded_data[std::get<0>(iter->first)].chinese;
			statusfile << "|";
			statusfile << loaded_data[std::get<0>(iter->first)].hungarian;
			statusfile << "|";
...
std::map<chinese::questions_hun::q_type, std::pair<size_t, double>> chinese::questions_hun::load_status_file(
	std::mt19937& gen,
...
) const
{
	...
	sequence_number = 400;
	std::fstream statusfile;
	statusfile.open("status_hun.txt", std::fstream::in);
	while(statusfile.good())
	{
		...
		std::string maybe_hungarian = tokens[1];
		...
		auto iter1 = iter0->second.find(maybe_hungarian);
		if(iter1 == iter0->second.end())
		{
...
bool chinese::questions_hun::ask_all_until_fail(int breaks) const
{
...
noreshuffle:
	while(1)
	{
		...
		{
			case DATATYPE_HUNGARIAN:
				asked_overlaps = &hungarian_overlaps;
				break;
			case DATATYPE_ENGLISH:
		...
		{
			case DATATYPE_HUNGARIAN:
				provided_overlaps = &hungarian_overlaps;
				break;
			case DATATYPE_ENGLISH:
...
repeat_question:
		...
		std::fstream statusfile;
		statusfile.open("status1_hun.txt", std::fstream::out | std::fstream::app);
		statusfile << current->get(DATATYPE_CHINESE);
		statusfile << "|";
		statusfile << current->get(DATATYPE_HUNGARIAN);
		statusfile << "|";
		...
		else
		{
			...
			if(iter != asked_overlaps->end())
			{
				for(size_t index: iter->second)
				{
					...
					statusfile << "|";
					statusfile << punishable->get(DATATYPE_HUNGARIAN);
					statusfile << "|";
		...
		else
		{
			if(!result)
			{
				...
				std::cout << ", HUN=" << loaded_data[std::get<0>(which_q)].get(DATATYPE_HUNGARIAN);
...
void chinese::questions_hun::populate_chinese_char_to_index()
{
...
bool chinese::questions_hun::ask_1_chinese_char(
	...
	while(good_answers_remaining.size())
	{
		...
		switch(asked)
		{
			...
			case DATATYPE_ENGLISH:
				...
				break;
			case DATATYPE_HUNGARIAN:
				gave = m_input.do_input_english(question_to_ask.str());
				break;
		}
		bool found = false;
		for(size_t i = 0; i < good_answers_remaining.size(); ++i)
		...
		if(!found)
		{
			...
			for(size_t i = 0; i < good_answers_remaining.size(); ++i)
			{
				...
				std::cout << good_answers_remaining[i]->get(DATATYPE_CHINESE) << " ("
				          << good_answers_remaining[i]->get(DATATYPE_HUNGARIAN) << ", "
...
void chinese::questions_hun::character_statistics_screen(
	std::map<chinese::questions_hun::q_type_char, std::pair<size_t, double>> const& recurrence_scores,
	int breaks,
	size_t sequence_number
) const
{
...
std::map<chinese::questions_hun::q_type_char, std::pair<size_t, double>> chinese::questions_hun::load_status_file_char(
	std::mt19937& gen,
	size_t& sequence_number,
	double score_improve_if_success,
	double score_deteriorate_if_fail,
...
{
	std::map<chinese::questions_hun::q_type_char, std::pair<size_t, double>> recurrence_scores;
	for(size_t q=0; q<loaded_data.size(); ++q)
	{
		...
		for(const std::string& chinese_char: chinese_chars)
		{
			...
			recurrence_scores[q_type_char(chinese_char, DATATYPE_HUNGARIAN)] = std::pair<size_t, double>(0, 0.0);
...
bool chinese::questions_hun::ask_all_chinese_chars(
	int breaks
) const
{
	std::mt19937 gen(std::chrono::steady_clock::now().time_since_epoch().count());
...
bool chinese::questions_hun::ask_1_chinese_char_easy(
	std::string question_number,
	std::string const& chinese_char,
	int breaks
) const
...
bool chinese::questions_hun::ask_all_chinese_chars_easy(
	int breaks
) const
{
	std::mt19937 gen(std::chrono::steady_clock::now().time_since_epoch().count());
...
std::map<chinese::questions_hun::q_type_char_easy, std::pair<size_t, double>> chinese::questions_hun::load_status_file_char_easy(
	std::mt19937& gen,
	size_t& sequence_number,
	double score_improve_if_success,
	double score_deteriorate_if_fail,
...
{
	std::map<chinese::questions_hun::q_type_char_easy, std::pair<size_t, double>> recurrence_scores;
	for(size_t q=0; q<loaded_data.size(); ++q)
	{
		std::vector<std::string> chinese_chars = m_input.split_into_chinese_characters(loaded_data[q].chinese);
		for(const std::string& chinese_char: chinese_chars)
...
void chinese::questions_hun::character_statistics_screen_easy(
	std::map<chinese::questions_hun::q_type_char_easy, std::pair<size_t, double>> const& recurrence_scores,
	int breaks,
	size_t sequence_number
) const
{
...
void chinese::questions_hun::init() const
{
	m_input.init();
}

void chinese::questions_hun::close() const
{
	m_input.close();
}

void chinese::questions_hun::system_pause() const
{
	m_input.system_pause();
}

int chinese::questions_hun::cols() const
{
	return m_input.cols();
}
